...
class loader{
    ...
    init_load(){
        this.load_cgf();
        return this.crayner.xhr(
            "GET",this.router+"/user/ajax?init="+encodeURIComponent(this.init_page),function(){
    ...
    pg(page, url){
        if (typeof (history.pushState) != "undefined") {
            ...
            this.crayner.xhr(
                "GET",this.router+"/user/ajax?load="+encodeURIComponent(url),function(){
                    document.getElementById('crayner').innerHTML+= this.responseText;
                }
            );
        } else {
